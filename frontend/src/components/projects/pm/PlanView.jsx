...
import PmTasksView, { TaskCard } from './PmTasksView'
import { PM_METHODS, TASK_STATUSES, taskIsLate, phaseProgress, daysUntil } from '../../../data/pmData'
...
function Gantt({ tasks, milestones }) {
  const dated = tasks.filter((t) => t.startDate && t.due)
  const points = [
        ...
        {dated.map((t) => {
          ...
          return (
            <div key={t.id} className="flex items-center gap-2">
              <div className="w-52 shrink-0 text-xs text-gray-600 truncate" title={t.title}>{t.title}</div>
              <div className="flex-1 relative h-5 bg-gray-50 rounded">
                <div className={`absolute top-1 bottom-1 rounded ${done ? 'bg-green-400' : late ? 'bg-red-400' : 'bg-blue-400'}`}
...
function SprintBoard({ pm, phase, onUpdatePhase, onUpdatePm, currentUserName }) {
  ...
  const inSprint = phase.tasks.filter((t) => t.sprintId === sprintId)
  const backlog = phase.tasks.filter((t) => !t.sprintId && t.status !== 'done')
  ...
  return (
    <div className="space-y-4">
        ...
        return (
          <div key={g.key} className="space-y-2">
            ...
            {items.length === 0 ? <div className="text-xs text-gray-300 pl-1">—</div>
              : items.map((t) => <TaskCard key={t.id} t={t} patch={(c) => patchTask(t.id, c)} currentUserName={currentUserName} />)}
          </div>
        )
      })}
      ...
      <div className="space-y-2 pt-2 border-t border-gray-200">
        ...
        {backlog.map((t) => (
          <div key={t.id} className="flex items-center gap-2">
            <div className="flex-1 min-w-0"><TaskCard t={t} patch={(c) => patchTask(t.id, c)} currentUserName={currentUserName} /></div>
...
export default function PlanView({ pm, phase, onUpdatePhase, onUpdatePm, currentUserName }) {
    ...
    <div className="space-y-4">
      ...
      {pm.method === 'waterfall' ? (
        <>
          ...
          <PmTasksView phase={phase} onUpdate={onUpdatePhase} currentUserName={currentUserName} />
        </>
      ) : (
        <SprintBoard pm={pm} phase={phase} onUpdatePhase={onUpdatePhase} onUpdatePm={onUpdatePm} currentUserName={currentUserName} />
      )}
    </div>
  )
}
