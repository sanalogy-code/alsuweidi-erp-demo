...
export const HELP_MODULES = [
  ...
  { key: 'projects', label: 'Projects', path: '/projects', icon: '🏗️',
    blurb: 'The delivery side: My Work, the PMO portfolio dashboard, Active Projects (health / reviews / risk report), resource planning, the All Projects database with Analytics, and a full per-project workspace (deliverables, site registers, FIDIC claims, authorities, reports).' },
...
export const HELP_TASKS = [
  ...
  { id: 'hr-approve-ts', module: 'hr', label: 'Approve my team’s timesheets', steps: ['HR → Team timesheets (managers), or HR Workspace → Time & leave → Timesheets (HR oversight).', 'Open a submitted week to see the per-day breakdown.', 'Approve, or reject with a reason. “Recently decided” confirms it landed.'], to: { path: '/hr', view: 'teamtimesheets' }, roles: ['hr', 'management', 'pm', 'admin'] },
  { id: 'hr-payroll', module: 'hr', label: 'Run the monthly payroll (WPS)', steps: ['HR → Pay & planning → Payroll.', 'Review the run (basic + allowances + overtime − deductions); note anyone flagged for an unsubmitted timesheet.', 'Draft → Generate SIF → Submitted → Paid; open a payslip to check the gratuity estimate.'], to: { path: '/hr', view: 'payroll' }, roles: ['hr', 'management', 'admin'] },
  { id: 'hr-renewals', module: 'hr', label: 'Check visa & document renewals', steps: ['HR → Compliance & admin → Renewals.', 'It lists visas, passports, contracts and dependent insurance expiring within 90 days or overdue.', 'Filter by document type; each row names the notified owner.'], to: { path: '/hr', view: 'renewals' }, roles: ['hr', 'management', 'admin'] },
  { id: 'hr-warning', module: 'hr', label: 'Issue a warning letter', steps: ['HR → Performance → Disciplinary.', 'Use “Issue warning” — employee, category, level (verbal/written/final), summary.', 'Print it on letterhead with the acknowledgement line.'], to: { path: '/hr', view: 'warnings' }, roles: ['hr', 'admin'] },
  { id: 'hr-headcount', module: 'hr', label: 'See headcount & attrition', steps: ['HR → Pay & planning → Headcount & attrition.', 'Read headcount by department, joiners vs leavers, annualized attrition and tenure buckets.', 'Exit reasons come from the Exit interviews log.'], to: { path: '/hr', view: 'headcount' }, roles: ['hr', 'management', 'admin'] },
  ...
  { id: 'pj-pmo', module: 'projects', label: 'Manage the whole portfolio (PMO)', steps: ['Projects → PMO.', 'Top strip = the 30-second read (click a RAG card to filter). Below it: only the projects that need attention and why, who is over-allocated across projects, and every contract deadline and milestone in the next 30 days.', 'Click a project to open its workspace.'], to: { path: '/projects', view: 'pmo' }, roles: ['management', 'pm', 'admin'] },
  ...
  { id: 'pj-open', module: 'projects', label: 'Open a project’s workspace', steps: ['Projects → All Projects (Database), find the project.', 'Click the row to open its workspace; the sidebar groups by intent (Plan & progress, Deliverables, Site, Claims & reports…) with the registers as tabs inside each group.', 'Use “Details” for the old record card.'], to: { path: '/projects', view: 'portfolio' } },
  { id: 'pj-analytics', module: 'projects', label: 'Learn from the project record (Analytics)', steps: ['Projects → Analytics (Database).', 'Computed patterns: which project types collect the claims/NCRs, what runs over its estimated hours, work done but not billed, supervision vs approved programme.', 'Sections marked ILLUSTRATIVE turn real when the historical projects are imported in Phase 2.'], to: { path: '/projects', view: 'dashboard' }, roles: ['management', 'pm', 'admin'] },
  ...
  // ---- Finance ----
  { id: 'fin-invoice', module: 'finance', label: 'Create a client invoice', steps: ['Financials → Money in → Invoices → New invoice (or the Record-an-invoice card on the Overview).', 'Link a project, enter the amount (5% VAT auto), set issue/due dates, note the attachment file name.', 'Send it; status runs draft → sent → partially paid → paid.'], to: { path: '/finance', view: 'invoices' }, roles: ['finance', 'management', 'admin'] },
  { id: 'fin-receipt', module: 'finance', label: 'Record a receipt against invoices', steps: ['Financials → Money in → Receipts → Record receipt (or the Record-a-payment card on the Overview).', 'Pick the client, enter date/reference/bank, and allocate the amount across their open invoices.', 'Allocation drives each invoice’s paid / partially-paid status.'], to: { path: '/finance', view: 'receipts' }, roles: ['finance', 'management', 'admin'] },
  { id: 'fin-credit', module: 'finance', label: 'Issue a credit note', steps: ['Financials → Money in → Invoices.', 'On a sent invoice with an outstanding balance, use “Credit note” (reason + amount, capped at outstanding).', 'It nets the client’s balance on their statement.'], to: { path: '/finance', view: 'invoices' }, roles: ['finance', 'management', 'admin'] },
  { id: 'fin-expense', module: 'finance', label: 'Log an expense with VAT', steps: ['Financials → Money out → Expenses → add expense (or the Log-an-expense card on the Overview).', 'Category, amount, VAT (auto 5%, editable) and a “non-recoverable” flag where it applies; optional project job-costing link.', 'It flows into the VAT working paper under Accountant.'], to: { path: '/finance', view: 'expenses' }, roles: ['finance', 'management', 'admin'] },
  { id: 'fin-payables', module: 'finance', label: 'Approve & pay supplier invoices', steps: ['Financials → Money out → Payables.', 'Approve a pending supplier/subconsultant invoice; “Start payment run” groups approved ones with a total.', 'Mark the run paid.'], to: { path: '/finance', view: 'payables' }, roles: ['finance', 'management', 'admin'] },
  { id: 'fin-monthend', module: 'finance', label: 'Run the month-end close', steps: ['Financials → Close & activity → Month-end close.', 'Pick the month; tick each item (bank rec, receipts allocated, expenses approved, VAT reviewed, payroll posted…).', 'Each tick stamps who and when.'], to: { path: '/finance', view: 'monthend' }, roles: ['finance', 'management', 'admin'] },
  { id: 'fin-reports', module: 'finance', label: 'Produce the monthly accountant reports', steps: ['Financials → Reports → Accountant.', 'Receivables aging (the chase list), client statements, and the VAT return working paper.', 'Each has a CSV export.'], to: { path: '/finance', view: 'accountant' }, roles: ['finance', 'management', 'admin'] },
  ...
  { id: 'mk-campaign', module: 'marketing', label: 'Group content into a campaign', steps: ['Marketing → Content → Campaigns → new campaign (name, goal, date range).', 'Attach content items to it.', 'The campaign card shows a published-progress line.'], to: { path: '/marketing', view: 'campaigns' }, roles: ['marketing', 'management', 'admin'] },
  { id: 'mk-event', module: 'marketing', label: 'Track an event or exhibition', steps: ['Marketing → Showcase → Events → add event.', 'Name, dates, venue, type, budget, owner.', 'After it, record outcomes and a leads count.'], to: { path: '/marketing', view: 'events' }, roles: ['marketing', 'management', 'admin'] },
  ...
  { id: 'it-queue', module: 'it', label: 'Work the IT request queue', steps: ['IT & Assets → Helpdesk → Request queue.', 'Approve → procure → fulfil, or reject with a reason.', 'Check SLA timers for anything running over target.'], to: { path: '/it', view: 'queue' }, roles: ['it', 'admin', 'management'] },
  { id: 'it-assets', module: 'it', label: 'Manage assets & licenses', steps: ['IT & Assets → Assets & software: the Assets tab to tag and assign hardware; the Licenses tab for software seats and renewals.', 'The renewal radar flags 60-day/overdue.', 'Installed software maps what’s on each asset and flags seat overruns.'], to: { path: '/it', view: 'assets' }, roles: ['it', 'admin', 'management'] },

  // ---- Office ----
  { id: 'of-corr', module: 'office', label: 'Log incoming / outgoing correspondence', steps: ['Office Admin → Documents & mail → Correspondence → log a letter.', 'Direction, party, project link, attachment; refs auto-number (IN/OUT-2026-NNN).', 'Reply-by tracking flags overdue items.'], to: { path: '/office', view: 'correspondence' }, roles: ['adminstaff', 'management', 'admin'] },
  { id: 'of-room', module: 'office', label: 'Book a meeting room', steps: ['Office Admin → Facilities → Meeting rooms.', 'Pick a room and date, set from/to and the purpose.', 'Overlapping times are rejected with a clash message.'], to: { path: '/office', view: 'rooms' }, roles: ['adminstaff', 'management', 'admin'] },
  { id: 'of-vehicle', module: 'office', label: 'Book a vehicle & log Salik', steps: ['Office Admin → Facilities → Vehicles & Salik.', 'Book a vehicle (driver, purpose, km out); “Log return” records km in.', 'Add Salik/fines per vehicle with a payroll-deduction flag.'], to: { path: '/office', view: 'vehicles' }, roles: ['adminstaff', 'management', 'admin'] },

  // ---- Admin ----
  { id: 'ad-users', module: 'admin', label: 'Manage users & roles', steps: ['Admin Center → Users & access → Users.', 'Add a user (mock invite), edit role, reset password, disable or delete.', 'Use the shield icon to grant per-user special access to a module.'], to: { path: '/admin', view: 'users' }, roles: ['admin', 'management'] },
  { id: 'ad-perms', module: 'admin', label: 'Edit the permissions matrix', steps: ['Admin Center → Users & access → Roles & permissions.', 'Click a cell to cycle — / View / Full for each role × module.', 'This mirrors the app’s gates and is the Phase 2 RBAC spec.'], to: { path: '/admin', view: 'roles' }, roles: ['admin', 'management'] },
  { id: 'ad-doa', module: 'admin', label: 'Set delegation-of-authority limits', steps: ['Admin Center → Users & access → Authority & access.', 'Click a limit to edit the AED threshold per action × level (staff / dept head / management).', 'Add grade/employment-type visibility rules below.'], to: { path: '/admin', view: 'governance' }, roles: ['admin', 'management'] },
  { id: 'ad-feedback', module: 'admin', label: 'Triage system feedback', steps: ['Admin Center → Oversight → System feedback.', 'Everything raised via the Feedback button lands here.', 'Move items through the triage statuses.'], to: { path: '/admin', view: 'feedback' }, roles: ['admin', 'management'] },
]
