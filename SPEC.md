...
| `CONTENT_ITEMS` | content calendar: `{ title, type (6 CONTENT_TYPES), channel, date, owner, status: idea → draft → pending_approval → approved → published, relatedProjectId (nullable FK), notes }` |
| `BRAND_ASSETS` | branding library (visible to everyone): `{ name, category (Logos/Templates/Guidelines/Stationery/Photography), format, sizeLabel, updatedDate, description }` — downloads mocked until Phase 2 storage |
...
`TIMESHEETS`: `{ employeeId, employeeName, weekStart (Sunday ISO), entries: [{ code: PROJECTS id | 'general'|'leave'|'training', hours: [7 numbers Sun..Sat] }], status: draft → submitted → approved | rejected, submittedDate, approvedBy, approvedDate, rejectReason }`. Week is currently company-wide Sun–Sat with Fri/Sat weekend — per-employee work-week patterns (Mon–Fri default, Sun–Thu Jordan staff) are backlogged. Helpers (`weekStartOf`, `fmtWeekRange`, `timesheetTotal`) live in the same file.

### Role groups (`dashboardData.js`)

`HR_STAFF_ROLES = ['hr', 'admin']` (process requests: inbox, certificates, complaints, holidays). `SENSITIVE_VIEW_ROLES = ['hr', 'admin', 'management']` (view sensitive data: visa/dependents/compensation tabs, renewals, payroll, attendance, leave planner, timesheet approvals, project financials). `MARKETING_VIEW_ROLES = ['marketing', 'management', 'admin']` (Marketing workspace — everything except the Branding library, which is visible to everyone). Client-side gating only — see §5.
...
3. **My timesheet** (`MyTimesheet`, Batch 4) — weekly project-linked timesheet: Sun–Sat grid (Fri/Sat weekend shaded), hours per project code per day plus General/Leave/Training codes, save draft / submit week, week navigation, "My recent weeks" history. Rejected weeks reopen editable with HR's reason. Home's "Fill Timesheet" quick action and "Fill this week" card deep-link here via router state. Requires a name-matched employee record.
4. **My requests** (`MyRequests`) — the employee's own leave + certificate + concern submissions in one filterable list with status chips. Anonymous concerns are deliberately not tracked here.
...
10b. **Timesheets** (`TimesheetApprovals`, Batch 4, `SENSITIVE_VIEW_ROLES`) — submitted weeks oldest-first with expandable per-day/per-code breakdown, approve / reject-with-reason (reason goes back to the employee, week reopens), and a missing-list for last week (drafts count as missing — they weren't submitted). Manager/PM approval chains are a pending decision; HR approves for now, mirroring leave.
...
2. **Project record** (`ProjectDetailModal`) — header with status chip and the 9-stage pipeline as a visual strip (`StagePipeline`; out-of-scope stages muted). Tabs render conditionally:
   - **Overview** — always; description, function, contract type, fund, sector/plot/area, contract & LOA state, contractor. **Batch 3:** a "Marketing sign-off" panel shows the marketing description (read-only here — Marketing owns it) and the description/photos gate status
...
Data: `IT_ASSETS` (assignedToId FK → HR `EMPLOYEES`), `SOFTWARE_LICENSES`, `IT_REQUESTS`, plus `ASSET_TYPES`, `ASSET_STATUS`, `IT_REQUEST_TYPES`, `IT_REQUEST_STATUS` enums. (Licenses view is read-only — "add license" is backlogged.)
...
1. **Inbox** (`MarketingInbox`) — one queue of everything waiting on Marketing, oldest first, badge on the nav. Auto-fed: **project created** (any path — CRM won-deal or direct) → `marketing_description` task; **project reaches final stage or a completion attempt is blocked** → `project_photos` task; **employee record created** (direct entry or approved joiner) → `employee_headshot` + `welcome_email` tasks. Actions inline: description editor modal (shows the PM's technical description for reference, saves to the project, completes the task), photo approval (sets `photosApproved`), headshot mark-done, and a welcome-email designer (prefilled draft; completing hands it to HR, who check and send — Marketing never sends directly, per decision).
2. **Content calendar** (`ContentCalendar`) — month grid (items as status-colored chips) + filterable month list, add/edit modal (type, channel, date, owner, optional related project), and the status workflow idea → draft → pending approval → approved → published, advanced inline.
3. **Portfolio** (`PortfolioView`) — Marketing's lens on the project list: portfolio-readiness per project (**ready** = description + approved photos + not confidential), stat cards (ready / missing description / confidential), readiness-type-status filters, write/edit descriptions (auto-completes any open description task), and the confidentiality toggle. (Hardening backlogged: confidentiality should be decided by the PM at project start and not be a one-click toggle.)
4. **Proposal builder** (`ProposalBuilder`) — two-step pack assembly: reference projects (type/function filters, auto-select portfolio-ready, **confidential projects never appear** — a note says how many are hidden) and team CVs (reuses `CvSearch` in selectable mode). Preview modal renders project sheets (marketing descriptions, with an inline warning where missing) + team cards; print-friendly. Branded PDF/slides export is Phase 2.
...
7. **Branding** (`BrandAssetsView`, everyone) — categorized brand asset library (logos incl. Arabic lockup, templates, guidelines, stationery, photography) with category filter chips; download buttons are demo-only until Phase 2 file storage.
...
- **Attendance is a mock dashboard.** The layout exists for sign-off; the actual fingerprint/card-reader feed and timesheet validation need the Phase 2 backend. ~~Project-linked weekly timesheets not built~~ — **built in Batch 4** (grid, submit, approvals, payroll hold flag), but the "unsubmitted blocks payroll" rule is display-only, and the work week is company-wide Sun–Sat: per-employee work-week patterns (Mon–Fri default, Sun–Thu for Jordan-based staff, other arrangements) auto-set from employment type are backlogged. Who approves timesheets (HR vs the project's PM) is also an open decision — hours are project-coded, so a PM split is natural once approval chains exist.
...
- **IT ↔ HR linkage is manual.** The IT module exists (requests, assets, licenses) but fulfilling a request doesn't auto-create an asset, and offboarding's equipment-return step doesn't auto-list the leaver's assigned assets — both are manual cross-checks today. Natural Phase 2 wiring once state lives in one database. Licenses are also read-only (no "add license" form yet — backlogged).
- **Marketing analytics are mock feeds** (LinkedIn, website) except the win rate, which is live from CRM deals. Welcome emails are design-only by decision (HR checks and sends — but sending itself needs the Phase 2 email capability). The confidentiality flag is too easy to flip (one click in Portfolio) — hardening backlogged: PM decides at project start, can't advance without deciding. Media/photo storage was **deliberately scoped out** of the Marketing module (Sana's call, 3 Jul — probably not necessary at all).
