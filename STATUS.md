...
**Last updated:** 2026-07-14 — **Batch 24: HR sidebar split, Me vs the HRMS** (Sana's 13-Jul point, grouping decided 14 Jul): the HR sidebar's mixed top section is now four labelled groups — **Me** (My HR / My timesheet / My appraisal / Training / My requests, + new-hire items), **My team** (manager-only: team timesheets/leave/appraisals), **Company** (People directory, Careers — everyone), and the existing **HR Workspace** intent groups. View keys unchanged (Guide deep-links intact). Also logged for consideration (no build yet): task-based timesheet approval (per-line approval by the task's supervisor, not just the line manager) and per-task resource allocation. Earlier: 2026-07-13 (late) — **Batch 23 continued: the project workspace sidebar got the intent-groups treatment too** (~25 rows → ~15 on a Design+Supervision project: per phase Plan & progress / Deliverables / Design gates / Site / Fees / Team; contract admin folded into Claims & reports / Authorities / Risks & meetings / Commercial & close-out — registers become pill tabs, deep-links unchanged). The Projects module sidebar reads (Sana's naming): **My Work · PMO · Active Projects** (tabs: Active projects / Project reviews / Risk report) **· Resources**, then Database: **All Projects · Analytics**. The in-app Guide's step texts were all rewritten to the new grouped navigation, with new PMO and Analytics recipes. Same day — **Batch 23: PMO dashboard + Portfolio analytics + honest branding.** New **Projects → PMO** (visual, show-first: RAG donut + big numbers, project tile grid with progress/SPI/billed bars and owner initials, person×week resource heatmap, 30-day deadline timeline; All/Mine scope — Mine = projects where you're DPM/CPM or on a phase team). **Database → Analytics replaces Record stats** (Sana: a stats page should answer "are we managing properly") — computed: claims/NCR clusters by project type, hours vs R0 estimate (Crew Training's 161% headline), earned-but-unbilled billing lag, supervision plan-vs-actual; plus clearly-labelled ILLUSTRATIVE Phase 2 shapes (duration slip by type, margin trend) that turn real when the ~140-project history is imported. **Branding page now contains only sourced facts** — the previous palette/fonts were invented demo data; now: logo red #c81516 + reversed white from the logo file, fonts/secondary palette explicitly "pending from Marketing". Also: favicon (logo emblem), title spelled ALSUWEIDI, Projects "Management" renamed **Active projects**. Earlier same day — **Batch 22: IA restructure** (intent-grouped sidebars for Finance / HR workspace / Office + verb-first action cards on the Finance overview — see Latest). Earlier, 11 Jul — **Batch 21: home page redesigned** ("launcher" — see Latest) and **Batch 20: the whole non-Phase-2 backlog cleared**. Earlier, 8 Jul — **Batch 19: in-app Guide** (a contextual `?` help hub in every navbar — see Latest). Earlier, 7 Jul night — **THE EVERYTHING LIST is built.** Batches 16d + 18a–g shipped in one sweep (see "Latest" below): attendance punch drill-down, the filters/date-pickers pass across ~24 registers, the notifications center, global search (Ctrl+K), My Week + management KPI home with board-pack print, delegation-of-authority + visibility rules, and the full persona sweep — accountant (receipts/credit notes/VAT/petty cash/payables/retention/month-end/audit trail), HR talent (appraisals/training/disciplinary/exits/attrition/grades), Marketing+BD (campaigns/approvals/usage/events/awards + bid gate/tender checklists/bid cost/competitors/debriefs), Office+IT (rooms/supplies/courier/vehicles/numbering + SLA/installed software/maintenance/access/status), and PM-site field tools (transmittals/RFIs/gate coordination/photo reports/safety log/quick daily entry). **What remains on BACKLOG.md is only:** items needing a decision or spec from Sana/management (Campaigns scope confirmation, grades scheme, PRO tenant question, Financials/GL scoping with the accountant, individuals-as-clients, appraisal model sign-off), Phase 2 backend work, and the standing non-code actions (show Phase 1 to management, IT VM conversation). Earlier same day: Batches 17a–b (PM visibility pass; default demo login **Samir Al Mazrouei / Management**), 16a–c, and the office-admin/special-access/filters morning batch. Decided: P6 stays the scheduling engine; resource planner is hired-employees-only.
...
**Latest (13 Jul 2026 — Batch 22, IA restructure — "I feel lost in there"):**
- Came out of Sana's usability conversation: the system is pretty but overwhelming — flat, equal-weight sidebars had grown to 12 items (Finance) and 16 (HR workspace). Diagnosis: we organize by data type; people think in intents (do something / check something / find something).
- **New pattern — sidebar = intent groups, registers = pill tabs inside the group** (shared `components/SubViewTabs.jsx`). Badge counts roll up onto the group. Old view keys unchanged, so the Guide's "Take me there" deep-links still land exactly right.
- **Finance**: Overview / Money in (Invoices · Receipts · Retention) / Money out (Expenses · Payables · Petty cash) / Reports (P&L · Revenue · Accountant) / Close & activity (Month-end · Activity). The Overview also gained **four verb-first action cards** — Record an invoice, Log an expense, Record a payment, Petty cash entry — that open the target register *with the add-form already open*.
- **HR workspace**: 16 items → Inbox / Time & leave / Pay & planning / Performance / Exits / Compliance & admin (every view keeps its original role gate). **Office**: 7 items → Documents & mail / Facilities / Registrations & licenses. **IT workspace**: 8 items → Helpdesk / Assets & software / System status. **Marketing workspace**: 9 items → Inbox / Content / Showcase / Tools & insights. **Admin Center**: 6 items → Overview / Users & access / Oversight.
- CRM and Projects keep their existing labeled sidebar groups — they were already the pattern. Decision noted for later: an external PDF guide is a credibility artifact, not a learning tool — teaching lives in the structure itself + the in-app Guide.

**Earlier (11 Jul 2026 evening — Batch 21, home page redesign):**
- Sana picked the "launcher" direction from three mockups: **centered greeting + a big search bar** (opens the Ctrl+K palette), **one amber attention line** composed live from your actual load (overdue tasks, approvals, deadlines, timesheet state — hidden when nothing needs you), **module tiles with live one-line statuses** ("7 open deals", "Timesheet due — fill this week", "AED 4.8M receivables"…), the management **Company KPIs** panel kept as-is, **news as three stock-photo media cards** (Unsplash placeholders until Phase 2 storage; gradient+emoji fallback if images fail) and an **Upcoming Holidays card** promoted from the footer (mini date tiles). Removed: the auto-rotating carousel, static 38.5h card, mock team members, dead quick links, quick-actions grid. Verified on mobile (375px) — no horizontal scroll, attention line restyled to wrap cleanly.

**Earlier (11 Jul 2026 — Batch 20, "do EVERYTHING that isn't Phase 2"):**
- **Code-quality consolidation, all 8 items:** shared `SidebarNav` (8 modules), shared `todayISO`/`fmtShortDate`/`daysUntil`/`fmtAED` usage (~60 inline date sites swapped), shared `nextId()` (~36 sites), shared `useRegisterFilter`/`RegisterFilterBar` (SiteView + GovernanceViews ×4 + DeliverablesView), **finance state lifted to App** (Home KPIs / PM dashboards / DMR-CMR / workspace fees now read live session invoices), per-invoice **credit-note netting** in aging + statements (`invoiceOutstandingNet`), **one audit-log mechanism** (`state/auditLog.js` — Finance actions land in Admin's Activity log; Finance → Activity is the module filter), denied leave now stamps `decidedDate` not `approvedDate`.
- **Good-to-haves:** new project auto-drafts a 10% mobilization invoice (draft only); project record gets a **Billing progress** block (invoiced vs fee, sensitive roles); P&L payroll line computed from HR employee gross instead of a mock.
- **Decision defaults built (need sign-off):** **PRO dashboard** (velocity, overdue, by-type — role-within-ALSUWEIDI shape) and **individuals as CRM clients** (`kind: 'individual'` toggle + chip, website/size hidden; seeded private-villa client).

**Earlier (8 Jul 2026 — Batch 19, in-app Guide / help system):**
- **A `?` in every navbar** opens one help hub, **contextual to the module you're on**. Three lenses: *For my role* (per-role orientation + your main modules), *How do I…* (~50 task recipes — 2–4 steps each + a "Take me there" button that deep-links to the exact screen), and a *Module map*. Content is one maintained file (`data/helpData.js`) at **module granularity** (per-page help would be a treadmill while screens still change); recipes are **role-gated**; press `?` anywhere to open. Supporting change: a `location.state.view` deep-link seam on Finance/CRM/IT/Marketing/Office/Admin/Projects so jumps land on the right sub-view even within the current module. Decision behind it: contextual `?` backed by module-level content, *not* per-page `?` — revisit per-field tooltips only after screens stabilise and real feedback shows specific confusion.

**Earlier (7 Jul 2026 night — Batches 16d + 18a–g, THE EVERYTHING LIST sweep):**
- **Notifications center** — bell + unread badge in every navbar: feed composed live from lifted state (tasks due/overdue, WIR/MIR/NCR/QA approvals waiting on you, claim-notice + 4.21 deadlines, own timesheet unsubmitted/rejected, team timesheets awaiting approval, HR pipeline-staffing queue, marketing tasks, new system feedback), module filter chips, per-user read state, click-through. Email/mobile delivery = Phase 2.
- **Global search** — Ctrl+K or the magnifier: people, live projects, companies, contacts, RFPs, screens. Full-text register search = Phase 2.
- **Home page** — **My Week** strip for everyone (open tasks + overdue, approvals waiting, deadlines, this week's timesheet state); **Company KPIs** for management/admin (utilization, bid win rate, receivables, project RAG health) + board-pack print.
- **Attendance punch drill-down (16d)** — Period-report rows expand to per-day In/Break/Resume/Out/Total grids whose Late/Missed/Leave flags reconcile with the summary counts.
- **Filters pass (18a)** — search + status + date-range on ~24 more registers across PM/HR/IT/Marketing/CRM/Finance ("many things are missing basics" — closed).
- **Accountant (18d)** — Receipts (allocate across invoices, drives paid states), Credit notes, per-expense VAT (real VAT working paper w/ recoverable vs non-recoverable vs legacy-estimate split), Petty cash + reconciliation, Supplier/Payables ledger + payment runs, Retention view, Month-end close checklist, session Audit trail (Activity view).
- **HR talent (18c)** — Appraisals (default self → manager → HR model, on-screen "to be agreed" note), Training (catalogue → enrollment → approval → auto-accomplishment), Disciplinary register (HR-only, letterhead print), Exit interviews + analytics, Headcount & attrition dashboard, optional Grades/bands (pending management).
- **Marketing + BD (18e)** — Campaigns (lightweight, to confirm with Sana), content approval records + pending-my-approval queue, pack usage log, Events, Awards; RFP bid/no-bid decision gate with rationale, tender doc checklist (progress chip on the register), bid cost tracking, competitor register + lost-to, lost-RFP debriefs rollup.
- **Office/ODC + IT (18f)** — Meeting rooms (clash detection), Supplies, Courier & dispatch, Vehicles + Salik/fines, Document-numbering reference; IT SLA timers, Installed software (seat-overrun flags), Preventive maintenance, Access requests (against the role matrix, record-only), System status board.
- **PM/site field tools (18g)** — Drawing transmittals (TRN refs, printable sign-off), RFI register (distinct from WIRs, cost/time flags), per-gate coordination checklists, 4.21 photo-report builder, HSE safety observation log + LTI counter, mobile-first Quick daily entry.
- **Admin (18b)** — Authority & access: delegation-of-authority matrix (click-to-edit AED limits) + grade/employment-type visibility rules (display-only until Phase 2 auth).

**Earlier (7 Jul 2026 evening — Batches 17a–b, Sana's PM visibility pass):**
- **Task table** — every phase's Plan & tasks defaults to a CLEAR aligned-column table (shaded container): task, status, priority, **assigned to**, **assigned by** (recorded at creation, phase lead for seeds), due — every column sorts; a row click expands the full editing card. New project-level **All tasks** view: every phase's tasks in one table with search + status/priority/assignee filters.
- **The board is a real board** — To do / In progress / Done side-by-side columns with compact cards (also on sprint boards).
- **Risk report** (Projects → Project Management) — portfolio dashboard: live/high-critical/realized/overdue-review cards, every project's register sorted worst-first with severity chips (P×I); each project's own Risks page gets a what's-bad summary strip.
- **Meetings log discussion** — free-text "what was discussed" notes on log + editable on existing meetings (PM-59 seeded with real minutes).
- **Gantt reads like a calendar** — month labels + gridlines; the scale focuses on the working window (long-achieved milestones no longer stretch the axis into uselessness).
- **Methodology is a setup decision** (Sana's call: "choose one at the beginning") — picked on the New-project form; shown in the workspace as a fixed "chosen at project setup" chip with change behind a confirm.
- **Management dashboard money** — "Labour cost to date" card + per-project **Cost (est.)** (hours × AED 210 blended) and **Invoiced / fee (%)** columns, sensitive roles only.
- **Default demo login = Samir Al Mazrouei / Management** — one persona for management walkthroughs: every module visible AND a real employee behind it (timesheet, My Work, approvals); his last week is seeded submitted so the lockout never interrupts. PM work is **parked** per Sana until further notice.

**Earlier (7 Jul 2026 — Batch 16c, pipeline staffing → HR):**
- **Pipeline staffing requests**: on any live RFP (CRM → Proposals), "Request staffing" raises an expected need — role × headcount, needed-by, note, contingent on award. It lands in an amber **Pipeline intake** queue at the top of **HR → Staff planning**; HR accepts it into the hiring plan (auto "Pipeline: <RFP>" reference + contingent note) or declines; handled requests keep a history. Seeds: two requests on the Jubail Research Facility RFP. The resource planner stays hired-employees-only per Sana's rule.

**Earlier (7 Jul 2026 — Batch 16b, construction feedback register):**
