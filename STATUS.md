...
- **HR workspace**: 16 items → Inbox / Time & leave / Pay & planning / Performance / Exits / Compliance & admin (every view keeps its original role gate). **Office**: 7 items → Documents & mail / Facilities / Registrations & licenses.
- CRM left as-is (already grouped by Sales / My Work / Insights); Projects, IT, Marketing, Admin are small enough for now. Decision noted for later: an external PDF guide is a credibility artifact, not a learning tool — teaching lives in the structure itself + the in-app Guide.
