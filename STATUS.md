...
**Last updated:** 2026-07-07 evening — **Batches 17a–b shipped** (Sana's PM visibility pass, see "Latest" below): task TABLE + real kanban board + project-level All tasks, portfolio Risk report, meeting discussion notes, Gantt month scale, methodology as a setup-time decision, management-dashboard costs, and the default demo login is now **Samir Al Mazrouei / Management** (sees everything, no lockout interruption). **Sana has parked PM work for now.** **Next up, in order:** attendance punch drill-down (last unshipped Batch 16 item), the rest of the filters/date-pickers sweep, then THE EVERYTHING LIST in [BACKLOG.md](BACKLOG.md) top-down — notifications center and per-user access-override UI depth are the biggest. Also decided: P6 stays the scheduling engine (import its exports in Phase 2, never rebuild CPM); resource planner is hired-employees-only.
...
**Latest (7 Jul 2026 evening — Batches 17a–b, Sana's PM visibility pass):**
